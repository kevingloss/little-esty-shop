class Invoice < ApplicationRecord
  ...
  # WIP - AR
    ...
    #   # savings = invoice_items.joins(:discounts).where("invoice_items.quantity >= threshold").select("invoice_items.*, max((discounts.percent/100) * invoice_items.quantity * invoice_items.unit_price) as savings").group("invoice_items.id, discounts.id").sum(&:savings)
    # end
